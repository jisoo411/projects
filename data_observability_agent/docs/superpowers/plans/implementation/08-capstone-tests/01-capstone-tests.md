# Capstone Integration Tests Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Write end-to-end integration tests that exercise the full request path — from HTTP request through guardrails, orchestrator, and guardrails again, to SSE response. Covers 5 RAG retrieval scenarios (asserting structured `airflow:` and `dbcheck:` URI format) and 4 abuse prevention scenarios.

**Architecture:** Uses a fully wired FastAPI `TestClient` (with lifespan) patching only the external calls: Airflow REST API, OpenAI, Cohere, and asyncpg. All internal wiring (guardrails → orchestrator → agents → RAG) is exercised without mocking. Tests assert that source URIs in the response match the exact structured format defined in the design spec.

**Tech Stack:** pytest-asyncio, httpx `AsyncClient`, pytest fixtures
...
### Task 1: Write Capstone Tests

**Files:**
- Create: `backend/tests/test_capstone.py`

- [ ] **Step 1: Write the capstone tests**

```python
# backend/tests/test_capstone.py
"""
...
async def test_both_uri_types_present_for_combined_query(app_with_mocks):
    ...
    with patch("agents.workflow_agent.retrieve",
        ...
        async with AsyncClient(transport=ASGITransport(app=app_with_mocks), base_url="http://test") as client:
            async with client.stream("POST", "/chat",
                                     json={"query": "pipeline and data quality summary for orders?"},
                                     headers={"X-API-Key": "test-key"}) as resp:
                body = b"".join([chunk async for chunk in resp.aiter_bytes()])
...
```bash
cd backend && python -m pytest tests/test_capstone.py -v
```
...
```bash
git add backend/tests/test_capstone.py
git commit -m "test: capstone integration tests (structured URIs, RAG, abuse prevention, rate limiting)"
```
